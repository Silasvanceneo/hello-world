import type { ChatChunk } from '@hello-world/shared';
import { parseSseEvents } from './sse.ts';

export async function* parseOpenAIResponsesStream(stream: ReadableStream<Uint8Array> | null): AsyncIterable<ChatChunk> {
  if (!stream) {
    yield { type: 'error', error: { code: 'network', message: 'Provider returned an empty stream.', retryable: true } };
    yield { type: 'done' };
    return;
  }

  for await (const event of parseSseEvents(stream)) {
    if (event.data === '[DONE]') {
      yield { type: 'done' };
      continue;
    }

    try {
      const parsed = JSON.parse(event.data) as {
        type?: string;
        delta?: string;
        text?: string;
        response?: { usage?: { input_tokens?: number; output_tokens?: number; total_tokens?: number } };
      };
      const type = parsed.type ?? event.event;
      if (type === 'response.output_text.delta' && parsed.delta) {
        yield { type: 'text-delta', text: parsed.delta };
      }
      if (parsed.response?.usage) {
        const usage = parsed.response.usage;
        yield {
          type: 'usage',
          usage: {
            promptTokens: usage.input_tokens ?? 0,
            completionTokens: usage.output_tokens ?? 0,
            totalTokens: usage.total_tokens ?? (usage.input_tokens ?? 0) + (usage.output_tokens ?? 0),
          },
        };
      }
    } catch (error) {
      yield {
        type: 'error',
        error: {
          code: 'unknown',
          message: error instanceof Error ? error.message : 'Invalid OpenAI Responses stream event.',
          retryable: false,
        },
      };
    }
  }

  yield { type: 'done' };
}
