...
test('provider store adds connections immutably and selects the first active connection', () => {
  ...
  const connection = createProviderConnection(
    { type: 'openai-compatible', name: '  OpenAI compatible  ', baseUrl: ' https://api.example.test/v1 ' },
...
test('provider store updates and deletes without mutating the original store', () => {
  ...
  const renamed = updateProviderConnection(store, 'ollama-1', { name: '  Local Ollama  ' }, { now: () => '2026-04-29T01:00:00.000Z' });
